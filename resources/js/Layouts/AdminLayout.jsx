...
import {
    ...
    Divider, Tooltip, Menu, MenuItem, Chip, Badge,
} from '@mui/material';
    ...
    return (
        <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'grey.50' }}>
            ...
            <Drawer variant="permanent" sx={{
                ...
                '& .MuiDrawer-paper': {
                    width: drawerWidth, transition: 'width 0.25s ease',
                    overflowX: 'hidden', bgcolor: '#1a2e1f',
                        ...
                        <Typography variant="h6" fontWeight={800} color="white">
                            ...
                            <Chip label="Admin" size="small"
                                sx={{ ml: 1, fontSize: '0.6rem', height: 18,
                                    bgcolor: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.4)' }} />
                        </Typography>
                    )}
                    {collapsed && (
                                    ...
                                    <ListItemButton component={Link} href={item.href}
                                        ...
                                        {!collapsed && (
                                            <Box sx={{ display: 'flex', alignItems: 'center',
                                                ...
                                                {badgeCount > 0 && (
                                                    <Chip label={badgeCount} size="small"
                                                        sx={{ fontSize: '0.6rem', height: 18, minWidth: 24,
                                                            bgcolor: '#d32f2f', color: 'white', fontWeight: 700, ml: 1 }} />
                                                )}
                                            </Box>
                                        )}
                                    </ListItemButton>
                ...
                <Box sx={{ p: 1.5 }}>
                    <ListItemButton onClick={e => setAnchorEl(e.currentTarget)}
                        ...
                        <Avatar sx={{ width: 30, height: 30, bgcolor: 'primary.dark',
                            fontSize: '0.8rem', fontWeight: 700, mr: collapsed ? 0 : 1.5 }}>
                            {auth?.user?.name?.[0]?.toUpperCase() ?? 'A'}
                        </Avatar>
                        {!collapsed && (
                            <Box sx={{ overflow: 'hidden' }}>
                                <Typography variant="caption" fontWeight={600} color="white" display="block" noWrap>
                                    {auth?.user?.name}
                                </Typography>
                                <Typography variant="caption" color="rgba(255,255,255,0.4)" display="block" noWrap>
                                    {auth?.user?.email}
                                </Typography>
                            </Box>
                        )}
